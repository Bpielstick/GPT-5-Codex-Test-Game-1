...
class GameMap:
    ...
    def set_tile_grass(self, tile_pos: Tuple[int, int]) -> None:
        x, y = tile_pos
        if not self.in_bounds((x, y)):
            return
        self.tiles[y][x] = Tile(TileType.GRASS, elevation=0.5, movement_cost=1.0)

    def carve_path(self, start: Tuple[int, int], end: Tuple[int, int], radius: int = 1) -> None:
        """Carve a grass corridor between ``start`` and ``end`` tiles."""
        ...
        err = dx + dy
        while True:
            self._stamp_circle((x0, y0), radius)
            if x0 == x1 and y0 == y1:
                break
            e2 = 2 * err
            if e2 >= dy:
    ...
    def _stamp_circle(self, center: Tuple[int, int], radius: int) -> None:
        cx, cy = center
